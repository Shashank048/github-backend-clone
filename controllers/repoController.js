...
async function createRepository  (req,res) {
    ...
    try{
        ...
        console.error("Error during repositories creation: ", err);
...
async function fetchRepositoryById ( req,res) {
    ...
    try{
        ...
        console.error("Error during repository: ", err);
        res.status(500).send("Server error!");
    }
};
...
async function fetchRepositoryByName  (req,res) {
    ...
    try{
        const repository = Repository.find({_id: name }).populate("owner").populate("issues");
        res.json(repository);
    }catch (err) {
        console.error("Error during repository: ", err);
        res.status(500).send("Server error!");
...
async function fetchRepositoriesForCurrentUser  (req,res){
    ...
    try{
        ...
        if(!repositories || repositories.length == 0){
            return res.status(404).json({error:"User Repositiries not found! "});
        }

        res.json({message: "Repositories found! ", repositories });
    }catch (err) {
        console.error("Error during user repositories: ", err);
        res.status(500).send("Server error!");
    }
};
...
async function deleteRepositoryById  (req,res) {
    ...
    try{
        ...
        console.error("Error during delete repository ", err);
        res.status(500).send("Server error!");
    }
};
